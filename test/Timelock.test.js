...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.Shit = await ShitToken.new(dev, { from: alice });
        this.timelock = await Timelock.new(bob, '28800', { from: alice }); //8hours
    });

    it('should not allow non-owner to do operation', async () => {
        await this.Shit.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.Shit.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.Shit.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.Shit.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.Shit.owner()).valueOf(), carol);
    });
    ...
    it('should also work with MasterChef', async () => {
        ...
        this.chef = await MasterChef.new(this.Shit.address, this.foam.address, dev, '1000', '0', '1', { from: alice });
        await this.Shit.transferOwnership(this.chef.address, { from: alice });
