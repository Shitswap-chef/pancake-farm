...
contract('MasterChef', ([alice, bob, carol, dev, , treasury, minter]) => {
    beforeEach(async () => {
        this.Shit = await ShitToken.new(treasury, { from: minter });
        ...
        this.chef = await MasterChef.new(this.Shit.address, this.foam.address, dev, '1000', '100', '1', { from: minter });
    ...
    it('real case', async () => {
      ...
      assert.equal((await this.Shit.balanceOf(alice)).toString(), '0');
      await this.chef.deposit(1, '20', { from: alice });
      await this.chef.withdraw(1, '20', { from: alice });
      assert.equal((await this.Shit.balanceOf(alice)).toString(), '263');
      ...
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      assert.equal((await this.Shit.balanceOf(alice)).toString(), '993');
    ...
    it('deposit/withdraw', async () => {
      ...
      assert.equal((await this.Shit.balanceOf(alice)).toString(), '999');
      assert.equal((await this.Shit.balanceOf(dev)).toString(), '100');
      ...
      assert.equal((await this.Shit.balanceOf(bob)).toString(), '125');
    ...
    it('updaate multiplier', async () => {
      ...
      await this.Shit.approve(this.chef.address, '100', { from: alice });
      await this.Shit.approve(this.chef.address, '100', { from: bob });
      ...
      assert.equal((await this.Shit.balanceOf(alice)).toString(), '700');
      assert.equal((await this.Shit.balanceOf(bob)).toString(), '150');
